...
void Paleta::Update(){
	...
	if(Keyboard::isKeyPressed(Keyboard::D) && m_Paleta.getPosition().x + Ancho < (640 + mAncho)){
		m_Paleta.move(velocidad,0);
	}
	...
	if(Keyboard::isKeyPressed(Keyboard::D) && Keyboard::isKeyPressed(Keyboard::LShift) && m_Paleta.getPosition().x + Ancho < (640 + mAncho)){
		m_Paleta.move(velocidad * 1.2,0);
	}
}
