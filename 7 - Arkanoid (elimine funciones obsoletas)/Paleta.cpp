...
void Paleta::Update(){
	...
	///			Opcion con <- y ->
	if(Keyboard::isKeyPressed(Keyboard::Left) && m_Paleta.getPosition().x - Ancho > (0 - mAncho)){
		m_Paleta.move(-velocidad,0);
	}
	if(Keyboard::isKeyPressed(Keyboard::Right) && m_Paleta.getPosition().x + Ancho < (820 + mAncho)){
		m_Paleta.move(velocidad,0);
	}
	if(Keyboard::isKeyPressed(Keyboard::Left) && Keyboard::isKeyPressed(Keyboard::Space) && m_Paleta.getPosition().x - Ancho > (0 - mAncho)){
		m_Paleta.move(-velocidad * 1.2,0);
	}
	if(Keyboard::isKeyPressed(Keyboard::Right) && Keyboard::isKeyPressed(Keyboard::Space) && m_Paleta.getPosition().x + Ancho < (820 + mAncho)){
		m_Paleta.move(velocidad * 1.2,0);
	}
}

void Paleta::Draw (RenderWindow &window){
	window.draw(m_Paleta);
}
