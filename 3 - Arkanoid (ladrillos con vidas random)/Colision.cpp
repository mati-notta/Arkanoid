...
Vector2f Colision::CalChoque(FloatRect bola, FloatRect ladrillo){
	...
	if (bola.left+bola.width < ladrillo.left) return {0,0}; // b1 est? completamente a la izquierda de b2
	if (bola.left > ladrillo.left + ladrillo.width) return {0,0}; // b1 est? completamente a la derecha de b2
	if (bola.top + bola.height < ladrillo.top) return {0,0}; // b1 est? completamente arriba de b2
	if (bola.top > ladrillo.top + ladrillo.height) return {0,0}; // b1 est? completamente abajo de b2
	float dx, dy;
